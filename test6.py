...
for i in range(100):
    ...
    A = S@D@S.T
    v,l,_ = lb6.metpot2k(A,1e-15,1e5)
...
A = S@D@S.T
SRH,DRH = lb6.diagRH(A,tol=1e-15,K=1e5)
...
for i in range(100):
    ...
    A = 0.5*(A+A.T)
    S,D = lb6.diagRH(A,tol=1e-15,K=1e5)
    ARH = S@D@S.T
    e = lb6.normaExacta(ARH-A,p='inf')#norma exacta devuelve una lista.
    if e < 1e-5:
        exitos += 1
assert exitos >= 95